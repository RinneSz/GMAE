...
class Batch():
    def __init__(self, idx, min_node_num, attn_bias, attn_edge_type, spatial_pos, in_degree, out_degree, x, edge_input, y):
    ...
    def to(self, device):
        ...
        if self.attn_edge_type is not None:
            self.attn_edge_type = self.attn_edge_type.to(device)
        if self.edge_input is not None:
...
def collator(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    ...
    if items[0].edge_input is not None:
        ...
        items = [(item.idx, item.attn_bias, item.attn_edge_type, item.spatial_pos, item.in_degree,
                  item.out_degree, item.x, item.edge_input, item.y) for item in items]
